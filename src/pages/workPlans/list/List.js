...
function WorkPlanList() {
  ...
  const pageName = "Work Plans";
  const routeStart = "/work-plan";
  const breadCrumbs = [
    {
      name: "Dashboard",
      link: "/dashboard",
                  ...
                  <div className="card-body p-0">
                    <Table
                      data={store.list.map((listData, index) => ({
                        assignedOn: utcToLocalTime(listData.assignedOn),
                        zone: listData.zone.name,
                        agriculturalPractice:
                          listData.agriculturalPractice.formulationName,
                        collaborator: listData.collaborator.name,
                        scannedTrees: listData.collaborator.name,
                        completedOn: listData.terminateOn
                          ? utcToLocalTime(listData.terminateOn)
                          : "N/A",
