...
function AgriculturalPracticeList() {
  const store = useSelector(selectAgriculturalPractice);
  const dispatch = useDispatch();
  const pageName = "Agricultural Practices";
  const routeStart = "/agricultural-practice";
  const breadCrumbs = [
    {
      name: "Dashboard",
      link: "/dashboard",
  ...
  const tableHeads = ["Formulation", "Method", "Added Date", "Status", ""];
                  ...
                  <div className="card-body p-0">
                    <Table
                      data={store.list.map((listData, index) => ({
                        formulationName: listData.formulationName,
                        method: methods[listData.method],
...
export default AgriculturalPracticeList;
